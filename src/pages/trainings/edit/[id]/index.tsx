...
import { useRoqClient, useTrainingFindFirst } from 'lib/roq';
...
function TrainingEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTrainingFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TrainingInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.training.update({
        data: values as RoqTypes.training,
        where: {
          id,
        },
      });
