...
import { useRoqClient, useBenefitFindFirst } from 'lib/roq';
...
function BenefitEditPage() {
  ...
  const { data, error, isLoading, mutate } = useBenefitFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: BenefitInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.benefit.update({
        data: values as RoqTypes.benefit,
        where: {
          id,
        },
      });
