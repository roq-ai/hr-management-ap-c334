...
import { useRoqClient, useTrainingFindManyWithCount } from 'lib/roq';
...
export function TrainingListPage(props: TrainingListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useTrainingFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.training.delete({ where: { id } });
      await mutate();
    } catch (error) {
