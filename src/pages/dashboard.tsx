...
import { useRoqPlatformUserProfiles, useDepartmentCount, useJobPostingCount, usePerformanceReviewCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const departmentStat = useDepartmentCount();

  const jobPostingStat = useJobPostingCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Departments',
      result: departmentStat?.data,
      isLoading: departmentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Job Postings',
      result: jobPostingStat?.data,
      isLoading: jobPostingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
