...
import { useRoqClient, useScheduleFindManyWithCount } from 'lib/roq';
...
export function ScheduleListPage(props: ScheduleListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useScheduleFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.schedule.delete({ where: { id } });
      await mutate();
    } catch (error) {
