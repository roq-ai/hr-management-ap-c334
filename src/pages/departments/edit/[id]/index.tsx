...
import { useRoqClient, useDepartmentFindFirst } from 'lib/roq';
...
function DepartmentEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDepartmentFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DepartmentInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.department.update({
        data: values as RoqTypes.department,
        where: {
          id,
        },
      });
